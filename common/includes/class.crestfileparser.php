 ...
 * @package EDK
 */
class CrestFileParserException extends Exception{}

/**
 * @package EDK
 */
class CrestFileParser
{
	private $error_ = array();
	/** @var string the contents of the CREST file to parse */
    private $crestFile;
    ...
    */
	function __construct($crestFile)
	{                
	...
	function parse()
	{
        ...
        try
        {
            $this->allKillmails = json_decode($this->crestFile);
        }
        catch(Exception $e)
        {
            throw new CrestFileParserException($e->getMessage(), $e->getCode());
        }
        
        $kmCount = 0;
        
        foreach($this->allKillmails as $km) {
            $this->killmailRepresentation = $km;
            ...
            $this->externalID = $this->killmailRepresentation->killID;
            $this->crestHash = $hash;
            ...
            if($checkHash->recordCount())
            {
                ...
                if($trust < 0)
                {
                    throw new CrestFileParserException("That mail has been deleted. Kill id was "
                .$this->getDupeID(), -4);
                }
                continue;//throw new CrestFileParserException("That killmail has already been posted <a href=\""
            ...
            else if($this->externalID)
            {
                ...
                if($qry->recordCount())
                {
                    $row = $qry->getRow();
                    continue;
                    //throw new CrestFileParserException("That killmail has already been posted <a href=\""
            ...
            if(config::get('filter_apply'))
            {
                ...
                if ($timestamp < $filterdate) {
                    ...
                    throw new CrestFileParserException("You are not allowed to post killmails older than" .$filterdate, -3);
                }
            }
            ...
            if (!$solarSystem->getName()) {
                throw new CrestFileParserException("Unknown solar system ID: ".$solarSystemID);
            }
            ...
            {
                throw new CrestFileParserException("Kill is a loss to NPCs only, but posting NPC kills is not allowed!", -5);
            }
     ...
     * @throws CrestFileParserException
     */
    protected function processVictim(&$Kill)
    {
        ...
        throw new CrestFileParserException("Insufficient victim information provided! Kill-ID: ".$this->externalID);
        } else if (!$victimDetails['corporationID'] && !$victimDetails['factionID']) {
                throw new CrestFileParserException("Insufficient victim corpiration information provided! Kill-ID: ".$this->externalID);
        }
    ...
    protected function processInvolved(&$Kill)
    {
        ...
        foreach($involvedParties AS $involvedParty)
        {
            if (!$involvedParty['shipTypeID']
                    ...
                    throw new CrestFileParserException("Error processing involved party. Kill-ID: ".$this->externalID);
            }
            ...
            if(!$involvedCharacterID && !$involvedParty['weaponTypeID'] && !$involvedParty['allianceID'])                        
            {
                ...
                if(!$Weapon->getName())
                {
                    throw new CrestFileParserException("Involved party is an NPC with a ship type not found in the database! Kill-ID: ".$killData->killID);
                }
